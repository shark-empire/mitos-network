...
fn track(interface_name: &str, kind: VpnKind) {
    ACTIVE.lock().unwrap().get_or_insert_with(HashMap::new).insert(interface_name.to_string(), kind);
}

fn untrack(interface_name: &str) -> Option<VpnKind> {
    ACTIVE.lock().unwrap().as_mut().and_then(|m| m.remove(interface_name))
}
...
pub fn connect(kind: VpnKind, config: &str, secrets: &dyn SecretsBackend, profile_id: &str) -> Result<VpnSession> {
...
pub fn disconnect(interface_name: &str) -> Result<()> {
    match untrack(interface_name) {
        ...
        None => Err(NetworkError::NotFound(format!("no active VPN session on '{interface_name}'"))),
    }
}
