...
impl NetworkManager {
    ...
    fn handle_hotplug(&mut self, ev: HotplugEvent) {
        match ev {
            HotplugEvent::LinkAdded(name) => {
                let _ = self
                    .devices
                    .refresh(&self.config.general.unmanaged_devices.clone());
            ...
            HotplugEvent::LinkChanged(name) | HotplugEvent::AddressChanged(name) => {
                let _ = self
                    ...
                    .refresh(&self.config.general.unmanaged_devices.clone());
                if let Some(dev) = self.devices.get(&name) {
                    self.events.broadcast(Event::DeviceStateChanged {
                        device: name,
                        state: dev.state,
                    });
                }
        ...
        match kind {
            ...
            TickKind::WifiScan => {
                let wifi_devices: Vec<String> = self
                    ...
                    .all()
                    .filter(|d| {
                        d.device_type == DeviceType::WiFi && d.state != DeviceState::Unmanaged
                    })
                    .map(|d| d.name.clone())
                    .collect();
                ...
                for (device_name, active) in self.active.clone() {
                    ...
                    if let Ok(Some(lease)) = crate::persistence::state::load_lease(&device_name) {
                        if std::time::SystemTime::now() >= lease.renewal_time() {
                            ...
                            {
                                let _ =
                                    crate::persistence::state::save_lease(&device_name, &new_lease);
                            }
                        }
                    }
                }
    ...
    fn try_autoconnect(&mut self, device_name: &str) {
        ...
        let chosen = if device.device_type == DeviceType::WiFi {
            ...
            let visible = crate::wifi::scanner::last_results(
                &self.config.wireless.ctrl_interface_dir,
                device_name,
            )
    ...
    fn activate_profile(&mut self, profile_id: &str) -> Response {
        ...
        let Some(device_name) = self.find_device_for_profile(&profile) else {
            return Response::Error(format!(
                "no available {} device for '{profile_id}'",
                profile.device_type
            ));
        };
        ...
        match crate::connection::activation::activate(&profile, device, self.secrets.as_ref()) {
            Ok(active) => {
                ...
                self.events
                    .broadcast(Event::ConnectionActivated(profile_id.to_string()));
                self.broadcast_state();
                Response::Ok
            }
            Err(e) => {
    ...
    fn deactivate_profile(&mut self, profile_id: &str) -> Response {
        let Some((device_name, _)) = self
            .active
            .iter()
            .find(|(_, a)| a.profile_id == profile_id)
    ...
    fn handle_request(&mut self, req: Request) -> Response {
        match req {
            ...
            Request::ListWifiNetworks { device } => {
                match crate::wifi::scanner::last_results(
                    &self.config.wireless.ctrl_interface_dir,
                    &device,
                ) {
                    Ok(nets) => Response::WifiNetworks(nets),
            ...
            Request::StartHotspot {
                ...
                match crate::sharing::hotspot::start(
                    &device,
                    &ssid,
                    passphrase.as_deref(),
                    uplink.as_deref(),
            ...
            Request::Reload => match crate::config::load(std::path::Path::new(
                crate::config::defaults_config_dir(),
            )) {
                Ok(cfg) => {
                    self.config = cfg;
                    Response::Ok
