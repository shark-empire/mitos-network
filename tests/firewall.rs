...
fn masquerade_and_forward_pairs_render_when_sharing() {
    ...
    let ruleset = render(&zones, &[], &["wlan0".to_string()], &[("eth0".to_string(), "wlan0".to_string())]);
...
fn outbound_rule_has_no_interface_qualifier() {
    let zones = builtin_zones();
    let rule = Rule { direction: Direction::Outbound, action: Action::Drop, ..Rule::allow_inbound_port("block-x", "public", Protocol::Tcp, 9999) };
    let ruleset = render(&zones, &[rule], &[], &[]);
